...
def home_experiment():
    return render_template('experiment/test.html')
...
@app.route('/experiment/posts/<int:post_id>')
def rest_list_posts_by_id(post_id):
    response = app.response_class(
        response=json.dumps(Post.find(post_id=post_id), indent=4, cls=PostEncoder),
        status=200,
        mimetype='application/json'
    )
    return response
